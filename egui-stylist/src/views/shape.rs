...
pub fn shape_view(style: &mut Style, ui: &mut Ui) {
    ...
    Grid::new("customization").show(ui, |ui| {
        ...
        ui.label("Text Cursor Stroke Width");
        DragValue::new(&mut style.visuals.text_cursor.stroke.width)
            .range(0.0f32..=50.0f32)
            .ui(ui);
        ui.end_row();

        ui.label("Text Cursor Stroke Color");
        ui.color_edit_button_srgba(&mut style.visuals.text_cursor.stroke.color);
        ui.end_row();

        ui.label("Toggle Text Cursor Preview");
        Checkbox::new(&mut style.visuals.text_cursor.preview, "").ui(ui);
        ui.end_row();
